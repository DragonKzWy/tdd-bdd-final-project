...
class TestProductModel(unittest.TestCase):
    ...
    def test_deserialize_with_attribute_error(self):
        """It should raise DataValidationError for AttributeError"""
        product = Product()
    ...
    def test_deserialize_with_type_error(self):
        """It should raise DataValidationError for TypeError"""
        product = Product()
        invalid_data = None
        with self.assertRaises(DataValidationError):
            product.deserialize(invalid_data)
    ...
    def test_deserialize_with_invalid_available_type(self):
        ...
        invalid_data = {
            ...
            "description": "Test",
            "price": "100.00",
            "available": "not_a_boolean",  # String instead of boolean
            "category": "CLOTHS"
        }
        with self.assertRaises(DataValidationError):
            product.deserialize(invalid_data)
    ...
    def test_deserialize_missing_key(self):
        """It should raise DataValidationError for missing keys"""
        product = Product()
    ...
    def test_find_by_price_with_string(self):
        """It should Find a Product by Price using string"""
        product = ProductFactory(price=Decimal("50.00"))
        product.create()

        found = Product.find_by_price("50.00")
        self.assertEqual(found.count(), 1)
        self.assertEqual(found[0].price, Decimal("50.00"))

    def test_find_product_not_found(self):
        """It should return None when product not found"""
        product = Product.find(99999)  # Non-existent ID
